...
narginchk(1,2);
if ~issiso(sys),
   ctrlMsgUtils.error('Control:general:FirstArgSISOModel','rlocfind')
end

% Create root locus plot
h = rlocusplot(sys);
if controllib.chart.internal.utils.isChart(h)
    currentObjectCandidateType = setCurrentObjectCandidateType(h,"axes");
end
...
if controllib.chart.internal.utils.isChart(h)
    setCurrentObjectCandidateType(h,currentObjectCandidateType);
end
