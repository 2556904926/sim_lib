...
%   1-dof case. The corresponding compensator structure is shown below for
%   the 2dof case:
%
%              .---------------------------------------------.
%              | u  .----------.  y_e                        |
...
if ndims(kest)>2 %#ok<ISMAT>
   error(message('Control:general:RequiresSingleModel','lqgtrack'))
elseif ~(isnumeric(k) && ismatrix(k))
   error(message('Control:design:lqgtrack6'))
end
...
if isfield(kest.OutputGroup_,'NoiseEstimate')
   % KEST outputs w[k|k] instead of y[k|k]. Ud=[] in this case
   ny = kest_nin-nu;
   Plant_nin = nu;
else
   ny = size(k,2) - nx;        % # of integrators = # measured plant outputs
   Plant_nin = kest_nin - ny;  % # of known (deterministic) plant inputs
end
Ts  = kest.Ts;

% Check if ny is properly set
if ny <= 0
   error(message('Control:design:lqgtrack1'))
end

% Check if dimensions of KEST are consistent
if kest_nout<nx || kest_nin<ny+nu
   error(message('Control:design:lqgtrack7',ny+nu,nx))
end
...
integrator_sys = ss(Aint,Bint,eye(ny),zeros(ny),Ts,'TimeUnit',kest.TimeUnit);
...
sw = ctrlMsgUtils.SuspendWarnings(...
   'Control:combination:connect9','Control:combination:connect10'); %#ok<NASGU>
if strcmpi(dof,'1dof')
   ...
   % 2dof case
   rNames  = "r" + (1:ny)';
   Sum = sumblk('%e=%r-%y',eNames,rNames,yNames); % Form a summation junction
...
if SingularFlag
   error(message('Control:design:lqgtrack4'))
end
...
nUd = length(UdNames);
lqgtrk.OutputName_ = uNames;
lqgtrk.OutputGroup_ = struct('Controls',1:nu);
if strcmpi(dof,'1dof')
   lqgtrk.InputName_ = [UdNames ; eNames];
   % Use public assign to remove empty groups
   lqgtrk.InputGroup = struct('KnownInput',1:nUd,'Error',nUd+1:nUd+ny);
else
   yNames = Kest_InputNames(Plant_nin+1:end);
   lqgtrk.InputName_ = [UdNames ; rNames; yNames];
   lqgtrk.InputGroup = struct('KnownInput',1:nUd,'Setpoint',nUd+1:nUd+ny,...
      'Measurement',nUd+ny+1:nUd+2*ny);
end