classdef ResponseLoopTransferGC < ctrluis.AbstractGC 
    %% Graphical component (GC) for Loop Transfer response
    %   
    %   GC = ResponseLoopTransferGC(TC) creates GC using the specified TC.
    %
    ...
    %       gc = controllib.widget.internal.responseplot.ResponseLoopTransferGC(<tc>);
    %       show(gc)
    %
    ...
    %
    %  See also controllib.widget.internal.responseplot.ResponseLoopTransferTC
    ...
    %% Constructor    
    methods
        function this = ResponseLoopTransferGC(tcpeer)
            this = this@ctrluis.AbstractGC(tcpeer);
            this.Name = "CSApp_ResponseLoopTransferDialog_" + matlab.lang.internal.uuid;
            this.Title = getString(message('Control:designerapp:ResponseTypeLoopTransferFunction'));
        end
    end    
    ...
    methods
        ...
        function [row, wt] = createIOWidgets(this, name, layout, row)
            ...
            this.LoopTransferGC.Padding = this.Padding;
            signalListPanel = createWidgets(this.LoopTransferGC, layout, row, [1 4]); 
        ...
        function widgets = createWidgets(this,parentLayout,row,col,varargin)
            ...
            if ~isempty(archIcon)
                ...
                archPanelLayout.RowHeight = {this.IconHeight};
    ...
    methods(Access=private)
        ...
        function cbHelpButton(this)
            if isSimulink(this.TCPeer.CDD.getArchitecture)
                ctrlguihelp('CSD_SL_EditResponseHelp','CSHelpWindow','CSHelpWindow');
            else
                ctrlguihelp('CSD_ML_EditResponseHelp','CSHelpWindow','CSHelpWindow');
            end
        end
    ...
    methods(Hidden = true)
        function widgets = qeGetWidgets(this)
            ...
            widgets.signalListPanels = qeGetWidgets(this.LoopTransferGC);
