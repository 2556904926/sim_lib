classdef ResponseSensitivityTransferGC < ctrluis.AbstractGC   
    %% Graphical component (GC) for Sensitivity Transfer response
    %   
    %   GC = ResponseSensitivityTransferGC(TC) creates GC using the
    %   specified TC.
    %
    ...
    %       gc = controllib.widget.internal.responseplot.ResponseSensitivityTransferGC(<tc>);
    %       show(gc)
    %
    ...
    %
    %  See also controllib.widget.internal.responseplot.ResponseSensitivityTransferTC
    %           controllib.widget.internal.responseplot.LoopTransferGC 
    ...
    %% Constructor    
    methods
        function this = ResponseSensitivityTransferGC(tcpeer)
            this = this@ctrluis.AbstractGC(tcpeer);
            this.Name = "CSDApp_ResponseSensitivityTransferDialog_" + matlab.lang.internal.uuid;
            this.Title = getString(message('Control:designerapp:ResponseTypeSensitivityTransferFunction'));
        end        
    end
    ...
    methods
        ...
        function [row, wt] = createIOWidgets(this, name, layout, row)
            ...
            % Signal panel lists
            row = row + 1;
            this.SensitivityTransferGC = createView(this.TCPeer.SensitivityTransferTC);
            this.SensitivityTransferGC.Padding = this.Padding;
            signalListPanel = createWidgets(this.SensitivityTransferGC,layout,row,[1 4]); 
    ...
    methods(Access = protected)
        function buildUI(this)
            ...
            % Set dialog size.
            this.UIFigure.Position(3:4) = [535 515];
    ...
    methods(Hidden = true)
        function widgets = qeGetWidgets(this)
            ...
            widgets.signalListPanels = qeGetWidgets(this.SensitivityTransferGC);
