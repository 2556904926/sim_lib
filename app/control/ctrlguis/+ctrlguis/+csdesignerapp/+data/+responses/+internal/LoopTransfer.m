classdef (Hidden) LoopTransfer < ctrlguis.csdesignerapp.data.responses.internal.ResponseConfiguration
    ...
    methods (Access = protected)
        ...
        function L = getResponse_(this,CL)
            % Computes scaled open-loop transfer from location
            LoopChannels = this.Location;
            if isempty(LoopChannels)
                % Grab all loop channels
                error(message('Controllib:general:UnexpectedError',...
                    'Cannot evaluate loop shaping requirement with unspecified "Location" property.'));
            end
        ...
        function DisplayText = getDisplayPreviewText_(this)
            ...
            TypeSection = ctrlguis.csdesignerapp.utils.internal.createDisplayText('type', ...
                getString(message('Control:designerapp:ResponseTypeLoopTransferFunction')));
