...
nominalIndex = getNominalIndex(this.Block);
...
% Set Callbacks
hb.MenuSelectedFcn = @(es,ed)localToggleBoundsMenu(es,hChart,m,getNominalIndex(this.Block));
hs.MenuSelectedFcn = @(es,ed)LocalToggleIndividualSystemsMenu(es,hChart,m,getNominalIndex(this.Block));

% Show only nominal response
arrayVisible = false(size(hChart.Responses(1).ArrayVisible));
arrayVisible(getNominalIndex(this.Block)) = true;
set(hChart.Responses,"ArrayVisible",arrayVisible);
set(hChart.Responses,"NominalIndex",getNominalIndex(this.Block));
end
...
hb.MenuSelectedFcn = @(es,ed) LocalToggleShowMenu(es,hChart,m,getNominalIndex(this.Block));
