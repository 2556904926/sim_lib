...
if any(ArraySizes==0)
   fprintf('\n%s =\n\n  %s\n\n',VarName,...
      getString(message('Control:ltiobject:pidDisplayArray1',ArrayDims(1:end-1))));
elseif nsys==1
   % Single PID
   fprintf('\n%s =\n',VarName)
   dispsys(Data,'  ')
   ...
   elseif Data.Ts==0
      MsgID = sprintf('Control:ltiobject:pidDisplayType%s1',getType(Data));
   else
      MsgID = sprintf('Control:ltiobject:pidDisplayType%s2',getType(Data));
   end
   ...
   elseif Data(1).Ts==0
      MsgID = 'Control:ltiobject:pidDisplayArray2';
   else
      MsgID = 'Control:ltiobject:pidDisplayArray3';
   end
