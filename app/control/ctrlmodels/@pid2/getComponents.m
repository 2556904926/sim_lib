function [out1, out2] = getComponents(PID2,looptype)
%%GETCOMPONENTS extracts two SISO control components from a 2-DOF PID controller
%
% [C,X] = getComponents(C2,LOOPTYPE), where C2 is a 2DOF PID controller object, returns
% two SISO control components C and X based on a given closed loop structure LOOPTYPE. 
% C is a PID object and X is a transfer function in ZPK form. The string LOOPTYPE specifies
% a loop structure for 2-DOF PID control implementation. The following loop structures
% are supported:
%
% 1. 'feedforward'
...
c1Data = createArray(size(Data),'ltipack.piddataP');
...
switch looptype
    ...
    case 'feedback'
        for ct=1:numel(Data)
            ...
            c1Data(ct) = pid(c1DataZPK,Options);
        end
    case 'filter'
        for ct=1:numel(Data)
            c1Data(ct) = make1DOF(Data(ct));
...
out1 = pid.make(c1Data);
out2 = zpk.make(xData);
out1.TimeUnit = PID2.TimeUnit;
out2.TimeUnit = PID2.TimeUnit;
out1.SamplingGrid = PID2.SamplingGrid;
out2.SamplingGrid = PID2.SamplingGrid;
end