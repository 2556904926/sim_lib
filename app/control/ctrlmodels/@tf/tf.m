...
%   Copyright 1986-2012 The MathWorks, Inc.
   ...
   methods
      ...
      function sys = tf(varargin)
         ...
         if Nsys==1
            ...
            Data = createArray(ArraySize,'ltipack.tfdata');
   ...
   methods (Access=protected)
      ...
      %% TRANSFORMATIONS
      function [sys,icmap] = absorbDelay_(sys,SCOPE)
         % Maps delays to poles in discrete time
         [sys,icmap] = absorbDelay_@ltipack.SystemArray(sys,SCOPE);
         % Remove trailing zeros for z^-1,q^-1 variables
         sys = trimNumDen(sys);
      end
      ...
      function [sys,Info] = balred_(sys,orders,Info,Options)
         ...
         rsys = tf(getrom(R,Order=orders,Method=Options.StateProjection));
