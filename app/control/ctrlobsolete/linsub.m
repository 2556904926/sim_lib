...
orig_in  = find_system(model,'findall','on',...
			     'MatchFilter',@Simulink.match.internal.filterOutInactiveVariantSubsystemChoices,... % look only inside active choice of VSS
                             'type','port','LinearAnalysisInput','on');
orig_out = find_system(model,'findall','on',...
			     'MatchFilter',@Simulink.match.internal.filterOutInactiveVariantSubsystemChoices,... % look only inside active choice of VSS
                             'type','port','LinearAnalysisOutput','on');
...
iospec = SetPotentialLinIO(model,des_in,des_out);
...
try
    if discflag
        s = dlinmod(model,options.SampleTime,x,u,'iospec', iospec);
    else
        s = linmod(model,x,u,'iospec', iospec);
    end
...
function iospec = SetPotentialLinIO(model,des_in,des_out)
iostruct = [];
iospec   = [];
for ct = 1:numel(des_in)
    iostruct(end+1).Block = get_param(des_in(ct),'Parent');
    iostruct(end).Port = get_param(des_in(ct),'PortNumber');
    iospec(end+1).Block = iostruct(end).Block;
    iospec(end).Port = iostruct(end).Port;
    iospec(end).Type = 'in';
    iospec(end).OpenLoop = false;
end
