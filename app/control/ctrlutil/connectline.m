...
action=varargin{1};
if ~any(strcmp(action,{'start';'copy';'open';'delete'}))
    ctrlMsgUtils.error('Control:utility:connectline1')
end
...
if nargin>1,
   ...
   try
       ...
       ctrlMsgUtils.error('Control:utility:connectline2')
   end
else
...
for ct = 1:length(AllLines), 
  ...
  if ~(srcType | dstType)
	...
	% Delete the first line going through the block (and its Branches)
	...
	for ctB=2:length(AllBranches),
	  %---Check if the branch is going through the line, if so,
