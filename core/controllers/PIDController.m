classdef PIDController < BaseController
    ...
    methods
        ...
        function adjustForPerformance(obj)
            ...
            if isfield(obj.design_params, 'rise_time') && ...
               isfield(obj.design_params, 'overshoot')
                
                target_rise = obj.design_params.rise_time;
                target_os = obj.design_params.overshoot;
                
                % 调整Kp（影响响应速度）
                if target_rise < 1
                    % 快速响应
                    obj.Kp = obj.Kp * 1.5;
                elseif target_rise > 3
                    % 慢速响应
                    obj.Kp = obj.Kp * 0.7;
                end
        ...
        function validate(obj)
            ...
            fprintf('\n=== PID控制器设计结果 ===\n');
            fprintf('设计方法: %s\n', obj.design_method);
            fprintf('参数: Kp=%.4f, Ti=%.4f, Td=%.4f\n', obj.Kp, obj.Ti, obj.Td);
            fprintf('等效: Kp=%.4f, Ki=%.4f, Kd=%.4f\n', obj.Kp, obj.Ki, obj.Kd);
            fprintf('性能: 上升时间=%.3fs, 超调量=%.1f%%, 调节时间=%.3fs\n', ...
                obj.performance.rise_time, obj.performance.overshoot, ...
                obj.performance.settling_time);
            fprintf('稳定: %s\n', string(obj.performance.stable));
            fprintf('相位裕度: %.1f°, 增益裕度: %.2f\n', ...
